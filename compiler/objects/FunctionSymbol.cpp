...
bool FunctionSymbol :: addWireConnection(string function_out, string function_in, int pos_out, int pos_in, string with_out, string name_wire, string out_name, string in_name){
	WireSymbol wire(function_out,function_in, pos_out, pos_in,with_out, name_wire, out_name, in_name);
...
void FunctionSymbol ::createFileModuleBase(){
	...
	for (int i = 0; i < this -> v_wire.size(); ++i){
		...
		this -> output_file_data += "\t wire " + with + v_wire.at(i).getNameWire() + ";";
